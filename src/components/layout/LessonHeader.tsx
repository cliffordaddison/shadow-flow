...
export function LessonHeader({ courseName, lessonName, progressLabel, progressValue, metrics }: Readonly<LessonHeaderProps>) {
  return (
    <div className="border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 min-h-[5.5rem] md:min-h-[5rem] mt-0.5 sm:mt-0">
      {/* MOBILE ONLY: Compact blocks — row 1: Course | Lesson; row 2: {progressLabel} + metrics (or inline on wider) */}
      <div className="md:hidden flex flex-col gap-2 py-2.5 px-3 min-h-[5.5rem]">
        <div className="flex flex-wrap items-stretch justify-center gap-2">
          <div className="flex-1 min-w-[min(100px,30%)] max-w-[45%] rounded-lg bg-slate-50/60 dark:bg-slate-800/50 px-2 py-1.5 text-center">
            <span className="block text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Course</span>
            <span className="block text-[11px] font-semibold text-slate-800 dark:text-slate-200 break-words leading-tight" title={courseName}>{courseName}</span>
          </div>
          <div className="hidden sm:block w-px self-stretch bg-slate-200 dark:bg-slate-600 shrink-0" aria-hidden />
          <div className="flex-1 min-w-[min(100px,30%)] max-w-[45%] rounded-lg bg-slate-50/60 dark:bg-slate-800/50 px-2 py-1.5 text-center">
            <span className="block text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Lesson</span>
            <span className="block text-[11px] font-semibold text-slate-800 dark:text-slate-200 break-words leading-tight" title={lessonName}>{lessonName}</span>
          </div>
          <div className="w-px self-stretch bg-slate-200 dark:bg-slate-600 shrink-0 sm:block hidden" aria-hidden />
          <div className="flex-1 min-w-[70px] rounded-lg bg-slate-50/60 dark:bg-slate-800/50 px-2 py-1.5 text-center shrink-0">
